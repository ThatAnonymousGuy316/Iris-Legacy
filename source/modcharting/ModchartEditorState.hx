...
class ModchartEditorState extends #if (PSYCH && PSYCHVERSION >= "0.7") backend.MusicBeatState #else MusicBeatState #end
{
   ...
   //pain
	...
	if(FlxG.sound.music != null)
        ...
        #if (PSYCH)
        strumLine = new FlxSprite(#if !(PSYCHVERSION >= "0.7") ClientPrefs.data.middleScroll #else ClientPrefs.data.middleScroll #end ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X, 50).makeGraphic(FlxG.width, 10);
    ...
    private function generateStaticArrows(player:Int):Void
    {
        ...
        var strumLineX:Float = #if (PSYCH && PSYCHVERSION >= "0.7") ClientPrefs.data.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X #elseif (PSYCH && PSYCHVERSION < "0.7") ClientPrefs.data.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X #elseif LEATHER utilities.Options.getData("middlescroll") #end ;
    ...
    public function saveModchartJson(?instance:ModchartMusicBeatState = null) : Void
    {
        ...
        var saveFolder:String = Sys.getCwd() + "/game/data/" + songName;

        if (!FileSystem.exists(saveFolder))
            FileSystem.createDirectory(saveFolder);

        File.saveContent(
            saveFolder + "/modchart.json",
            Json.stringify(instance.playfieldRenderer.modchart.data, null, "\t")
        );
